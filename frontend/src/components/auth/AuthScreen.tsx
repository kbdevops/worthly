...
export default function AuthScreen({ onAuthed }: { onAuthed: () => void }) {
  ...
  // Registration is closed on an exposed instance, so don't offer a tab that only 403s.
  // Defaults closed: if the probe fails, show the safer UI rather than the friendlier one.
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div>
            ...
            <input type="password" required value={password} onChange={e => setPassword(e.target.value)}
              ...
              minLength={mode === 'register' ? minLen : undefined}
              className="w-full px-3 py-2 rounded-lg text-sm text-slate-200 focus:outline-none"
              style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)' }} />
          </div>
